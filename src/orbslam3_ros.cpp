...
: nh_(nh), nhp_(nhp)
{
  ...
  pub_pose_ = nh_.advertise<geometry_msgs::PoseStamped>("orbslam3/pose", 1);
...
// ----------------------------------------------------------------------------
geometry_msgs::PoseStamped keep_pose;

void ROSWrapper::handle_tracking_results(const cv::Mat& _Tcm, const ros::Time TIME)
{
  ...
  posemsg_.header.stamp = TIME; //ros::Time::now();
  // posemsg_.pose = tf2::toMsg(Twb);
  if((tf2::toMsg(Twb).position.x!=0)&&(tf2::toMsg(Twb).position.y!=0)&&(tf2::toMsg(Twb).position.z!=0))
  {//若不为0,按当前输出，且更新keep_pose
    posemsg_.pose = tf2::toMsg(Twb);
    keep_pose.pose=tf2::toMsg(Twb);
  }else
  {
    posemsg_.pose=keep_pose.pose;//把上一刻的输出
  }
...
void ROSWrapper::slamsync()
{
  while (ros::ok()) {
    ...
    if (!buf_img1_.empty()) {
      ...
      time_pub=buf_img1_.front()->header.stamp;
      ...
      cv::Mat Tcm;
      Sophus::SE3f Tcm_SE3f;
      if (sensor_ == 0 || sensor_ == 3) {
        // Tcm = slam_->TrackMonocular(img1, t_img, imus);
        Tcm_SE3f = slam_->TrackMonocular(img1, t_img, imus);
        // ROS_INFO("lol!!!!!!!!!!!!!!!!!");
      }
      Eigen::Matrix4f Tcw_Matrix = Tcm_SE3f.matrix();
      cv::eigen2cv(Tcw_Matrix, Tcm);

      handle_tracking_results(Tcm,time_pub);
    }
  ...
  const sensor_msgs::ImageConstPtr& rightmsg)
{
  ...
  cv::Mat Tcm;
  Sophus::SE3f Tcm_SE3f;
  Tcm_SE3f= slam_->TrackStereo(left, right, leftmsg->header.stamp.toSec());
  Eigen::Matrix4f Tcw_Matrix = Tcm_SE3f.matrix();
  cv::eigen2cv(Tcw_Matrix, Tcm);
